...
master$total <- psum( master$IDPs.in.Camps.or.transit.camps,
                      ...
                      master$Collective.centres,
                      master$Informal.settlements,
...
master$private.setting <- psum( master$Rented.Hotel,
                                master$Rented.House ,
                                master$IDP.Owned.House,
                                master$With.Relative ,
                                master$With.HC.non.Relative,
...
# Recoding priority needs
#levels(master$priority.need.of.the.displaced.population.in.the.location)
...
#head(master)
#levels(master$Note)
#levels(master$assistance.previously.received)
#levels(master$sufficient.access.to.food)
...
# - # of IDP hosted in communal setting  -- a score can be given based on the size on the population
# a classification is computed on 5 classes using different algorythm
...
# print(classIntervals(master$total, n = 5, style = "jenks"), unique=FALSE)
# pal1 = brewer.pal(5,"Greens")
# plot(classIntervals(master$total, n = 5, style = "jenks"), pal=pal1, main="Classification per total population")
...
# - specific score depending on the date of displacement (pre-june /post-june).
##master$class.date  <-findCols(classIntervals(as.numeric(master$Date.Displacement), n = 5, style = "jenks"))
...
## Try to find place with same name but different master id
#extractplacecountplace <- aggregate(cbind(total) ~ Place ,  data = master, FUN = sum, na.rm = TRUE)
...
#IDPs.in.Camps.or.transit.camps ;  IDPs.in.Camps.or.transit.camps
#Rented.Hotel ;  Rented.or.Owned.Accomodation
#Rented.House ;   Rented.or.Owned.Accomodation
#IDP.Owned.House ;   Rented.or.Owned.Accomodation
#With.Relative ;   Hosted.Accomodation
#With.HC.non.Relative ; Hosted.Accomodation
#School.Building ; Self-settled.in.public.building
#Mosques.Holly.Shrines  ; Self-settled.in.public.building
#Abandoned.public.buildings.under.construction ; Self-settled.in.public.building
#Collective.centres ; Collective.centres
#Informal.settlements  ; Informal.settlements
#Military.Camps ; Informal.settlements
#Unknown.or.other  ; Unknown.or.other

master$Hosted.or.Owned.Accomodation <- psum( master$IDP.Owned.House,
                                             master$With.Relative,
                                             master$With.HC.non.Relative, 
                                             na.rm=TRUE)

master$Rented.Accomodation <- psum(master$Rented.House,
                                   master$Rented.Hotel,
                                   na.rm=TRUE)

master$Organised.site   <- psum( master$Collective.centres,
                                 master$Mosques.Holly.Shrines,
                                 master$Military.Camps,  
                                 na.rm=TRUE)

master$Improvised.site   <- psum( master$IDPs.in.Camps.or.transit.camps,
                                  master$Abandoned.public.buildings.under.construction,
                                  na.rm=TRUE)
